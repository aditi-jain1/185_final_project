...
def compute_offline_preference_loss(
    *,
    ...
    reference_free_margin: float = 1.0,
    reference_free_margin_mode: str = "sum",
    reference_free_loss_type: str = "hinge",
) -> OfflineLossOutput:
    ...
    if algo == "dpo":
        ...
        chosen_rewards, _ = torch.sort(chosen_rewards, descending=True)
        rejected_rewards, _ = torch.sort(rejected_rewards, descending=True)
    ...
    elif algo == "reference_free":
        if reference_free_margin_mode not in {"sum", "mean"}:
            raise ValueError(
                f"reference_free_margin_mode must be 'sum' or 'mean', got {reference_free_margin_mode}"
            )
        if reference_free_loss_type not in {"hinge", "softplus"}:
            raise ValueError(
                f"reference_free_loss_type must be 'hinge' or 'softplus', got {reference_free_loss_type}"
            )
        margin = policy_margin_sum if reference_free_margin_mode == "sum" else policy_margin_mean
        scaled_margin = beta * margin
        if reference_free_loss_type == "hinge":
            losses = F.relu(reference_free_margin - scaled_margin)
        else:
            losses = F.softplus(reference_free_margin - scaled_margin)
        metrics.update(
            {
                "preference/reference_free_margin_target": float(reference_free_margin),
                "preference/reference_free_scaled_margin_mean": float(scaled_margin.detach().mean().item()),
                "preference/reference_free_accuracy": float((scaled_margin.detach() > reference_free_margin).float().mean().item()),
            }
        )
    elif algo == "apo_zero":
        # push chosen up and pull rejected down
        losses_chosen  = 1 - F.sigmoid(beta * policy_scores.chosen_logp_sum)
        losses_rejected = F.sigmoid(beta * policy_scores.rejected_logp_sum)
        losses = losses_chosen + losses_rejected
        metrics.update({
            "preference/apo_chosen_loss_mean": float(losses_chosen.detach().mean().item()),
            "preference/apo_rejected_loss_mean": float(losses_rejected.detach().mean().item()),
        })
    elif algo == "apo_down":
        ...
        ref_margin_sum = reference_scores.chosen_logp_sum - reference_scores.rejected_logp_sum
        losses_chosen  = 1 - F.sigmoid(beta * (policy_scores.chosen_logp_sum - reference_scores.chosen_logp_sum))
        losses_rejected = F.sigmoid(beta * (policy_scores.rejected_logp_sum - reference_scores.rejected_logp_sum))
        losses = losses_chosen + losses_rejected
        metrics.update({
            "preference/reference_margin_sum_mean": float(ref_margin_sum.detach().mean().item()),
            "preference/apo_chosen_loss_mean": float(losses_chosen.detach().mean().item()),

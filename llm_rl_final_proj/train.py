...
def parse_args() -> TrainConfig:
    ...
    ap.add_argument("--use_confidence_weighting", action=argparse.BooleanOptionalAction, default=TrainConfig.use_confidence_weighting)
    ap.add_argument("--confidence_weight_scale", type=float, default=TrainConfig.confidence_weight_scale)
    ap.add_argument("--confidence_weight_min", type=float, default=TrainConfig.confidence_weight_min)

    ap.add_argument("--reference_free_margin", type=float, default=TrainConfig.reference_free_margin)
    ap.add_argument(
        "--reference_free_margin_mode",
        type=str,
        default=TrainConfig.reference_free_margin_mode,
        choices=["sum", "mean"],
    )
    ap.add_argument(
        "--reference_free_loss_type",
        type=str,
        default=TrainConfig.reference_free_loss_type,
...
def main() -> None:
    ...
    need_reference = cfg.algo in {"dpo", "ipo", "aot", "apo_down"}
    ...
    def _build_confidence_example_weights(batch, cfg: TrainConfig) -> torch.Tensor | None:
        if not cfg.use_confidence_weighting:
            return None
        weights = batch.avg_confidence * cfg.confidence_weight_scale
        return torch.clamp(weights, min=cfg.confidence_weight_min)
    ...
    while optimizer_step < estimated_total_optimizer_steps:
        for batch in train_loader:
            ...
            example_weights = _build_confidence_example_weights(batch, cfg)
            loss_out = compute_offline_preference_loss(
                ...
                reference_free_margin=cfg.reference_free_margin,
                reference_free_margin_mode=cfg.reference_free_margin_mode,
                reference_free_loss_type=cfg.reference_free_loss_type,
            )
